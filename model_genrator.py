...
class ModelTrainer:
    ...
    def _train_single_model(self, name, model, X_train, X_val, X_test,
        ...
        # Save home model
        home_model_path = f'models/{name}_home_model.pkl'
        try:
            joblib.dump(home_model, home_model_path)
            print(f"  ✓ Saved home model: {home_model_path}")
        except Exception as e:
            print(f"  ⚠️ Failed to save home model: {e}")
        ...
        # Save away model
        away_model_path = f'models/{name}_away_model.pkl'
        try:
            joblib.dump(away_model, away_model_path)
            print(f"  ✓ Saved away model: {away_model_path}")
        except Exception as e:
            print(f"  ⚠️ Failed to save away model: {e}")
...
class FootballPredictor:
    ...
    def run(self, filepath, train_ratio=0.6, val_ratio=0.2):
        ...
        try:
            ...
            print("  • poisson_home_model.pkl")
            print("  • poisson_away_model.pkl")
            print("  • ridge_home_model.pkl")
            print("  • ridge_away_model.pkl")
            print("  • lasso_home_model.pkl")
            print("  • lasso_away_model.pkl")
            print("  • scaler.pkl")
...
if __name__ == "__main__":
    ...
    if results:
        print("\n" + "="*80)
        print("✅ ALL DONE! Check your directory for CSV files and /models for .pkl files")
        print("="*80)